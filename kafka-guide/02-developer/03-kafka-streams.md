...
| Restore takes minutes/hours | Big changelog, no local state | `restore-rate`, `restore-remaining-records-total` (stream-state-metrics, 3.5+), `active-restore-ratio` (stream-thread-metrics), `state` = REBALANCING | `num.standby.replicas=1`, persistent volumes, `topology.optimization=all` |
