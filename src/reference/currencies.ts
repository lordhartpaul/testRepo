/**
 * ISO 4217 active currency codes with their minor unit (number of decimals).
 *
 * The minor unit drives two things the converter cares about:
 *  - MT validation: field 32A/33B amounts may not carry more decimals than the
 *    currency allows (SWIFT network validated rule C03/T40).
 *  - MX rendering: ISO 20022 `ActiveCurrencyAndAmount` is fraction-constrained
 *    by the same table.
 *
 * `null` means the code has no defined minor unit (precious metals, testing
 * codes); decimal checks are skipped and reported as informational.
 */
export const CURRENCY_MINOR_UNITS: Readonly<Record<string, number | null>> = Object.freeze({
  ...
  WST: 2,
  XAF: 0, XCD: 2, XCG: 2, XDR: null, XOF: 0, XPF: 0,
...
export function isKnownCurrency(code: string): boolean {
  return Object.prototype.hasOwnProperty.call(CURRENCY_MINOR_UNITS, code.toUpperCase());
}

/** Decimals allowed for a currency; `null` when the currency defines none. */
export function minorUnits(code: string): number | null | undefined {
  return CURRENCY_MINOR_UNITS[code.toUpperCase()];
}
